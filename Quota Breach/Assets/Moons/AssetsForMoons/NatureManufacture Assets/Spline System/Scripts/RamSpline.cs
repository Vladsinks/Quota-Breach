...
public class RamSpline : MonoBehaviour
{
    ...
    public float terrainMeshSmoothX = 2;
    public float terrainMeshSmoothZ = 10;
    ...
    public int currentWorkTerrain = 0;
    ...
    public void GenerateBeginningParentBased()
    {
        ...
        pos = Vector3.Lerp(beginningSpline.pointsDown[beginningConnectionID],
                beginningSpline.pointsUp[beginningConnectionID],
    ...
    public void GenerateEndingParentBased()
    {
        ...
        pos = Vector3.Lerp(endingSpline.pointsDown[endingConnectionID], endingSpline.pointsUp[endingConnectionID],
    ...
    public void GenerateSpline(List<RamSpline> generatedSplines = null)
    {
        ...
        if (beginningSpline != null && beginningSpline.controlPointsRotations.Count > 0)
            controlPointsRotations[0] = Quaternion.Inverse(controlPointsOrientation[0]) *
                                        (beginningSpline.controlPointsOrientation[
                                            beginningSpline.controlPointsOrientation.Count - 1]);

        if (endingSpline != null && endingSpline.controlPointsRotations.Count > 0)
            controlPointsRotations[controlPointsRotations.Count - 1] =
                Quaternion.Inverse(controlPointsOrientation[controlPointsOrientation.Count - 1]) *
                (endingSpline.controlPointsOrientation[0]); // * endingSpline.controlPointsRotations [0]);
        ...
        for (int i = 0; i < controlPointsUp.Count; i++)
        {
            ...
            CalculateCatmullRomSpline(controlPointsUp, i, ref carvePointsUp);
        }
        ...
        for (int i = 0; i < controlPointsDown.Count; i++)
        {
            ...
            CalculateCatmullRomSpline(controlPointsDown, i, ref carvePointsDown);
        }
        ...
        if (generatedSplines != null)
        {
            ...
            {
                //Debug.Log(gameObject.name + " - " + item.name);
                if (item != null && !generatedSplines.Contains(item))
                {
                    //Debug.Log(gameObject.name + " gen - " + item.name);
                    if (item.beginningSpline == this || item.endingSpline == this)
                    {
                        item.GenerateSpline(generatedSplines);
                    }
    ...
    void CalculateCatmullRomSideSplines(List<Vector4> controlPoints, int pos)
    {
        ...
        for (int tValue = 0; tValue <= tValueMax; tValue++)
        {
            ...
            orientation *= Quaternion.Lerp(controlPointsRotations[pos], controlPointsRotations[ClampListPos(pos + 1)],
                tValue);
    ...
    void CalculatePointPosition(List<Vector3> controlPoints, int pos, Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3,
        float t, ref List<Vector3> points)
    {
    ...
    void CalculatePointParameters(List<Vector4> controlPoints, int pos, Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3,
        float t)
    {
        Vector3 newPos = GetCatmullRomPosition(t, p0, p1, p2, p3);
        ...
        for (int i = 0; i < pointsDown.Count; i++)
        {
            ...
            for (int j = 0; j < vertsInShape; j++)
            {
                    ...
                    beginningSpline.normalsEnding != null)
                {
                    ...
                    vertices[id] =
                        beginningSpline.verticesEnding[
                            Mathf.Clamp(j + pos2, 0, beginningSpline.verticesEnding.Count - 1)] +
                        beginningSpline.transform.position - transform.position;
                ...
                else
                {
                    ...
                    RaycastHit hit;
                    if (Physics.Raycast(vertices[id] + transform.position + Vector3.up * 5, Vector3.down, out hit, 1000,
                            snapMask.value))
                    {
                        checkTerrain = hit.collider.gameObject.GetComponent<Terrain>();
                        if (checkTerrain && !terrainsUnder.Contains(checkTerrain))
                        {
                        ...
                        vertices[id] = Vector3.Lerp(vertices[id],
                            ...
                            (Mathf.Sin(Mathf.PI * snaps[i] - Mathf.PI * 0.5f) + 1) * 0.5f);
                    }
                    ...
                    {
                        RaycastHit hit2;
                        if (Physics.Raycast(points[i] + transform.position + Vector3.up * 5, Vector3.down, out hit2,
                                1000, snapMask.value))
                        {
                    ...
                    vertices[id] += orientations[i] * Vector3.up * Mathf.Lerp(controlPointsMeshCurves[Mathf.FloorToInt(lerpValues[i])].Evaluate(pos),
                ...
                if (i == pointsDown.Count - 1 && endingSpline != null && endingSpline.verticesBeginning != null)
                {
                    for (int k = 1; k < 5; k++)
                    {
                        vertices[id - vertsInShape * k].y =
                            (vertices[id - vertsInShape * (k - 1)].y + vertices[id - vertsInShape * k].y) * 0.5f;
                    }
                }
                ...
                if (beginnigChildSplines.Count > 0 && i <= lerpDistance)
                {
                    ...
                    foreach (var item in beginnigChildSplines)
                    {
                        if (item == null)
                            continue;
                        if (Mathf.CeilToInt(item.endingMaxWidth * (vertsInShape - 1)) >= j &&
                ...
                if (i >= pointsDown.Count - lerpDistance - 1 && endingChildSplines.Count > 0)
                {
                    ...
                    foreach (var item in endingChildSplines)
                    {
                        if (item == null)
                            continue;
                        if (Mathf.CeilToInt(item.beginningMaxWidth * (vertsInShape - 1)) >= j &&
    ...
    public void GenerateMeshParts(Mesh baseMesh)
    {
        ...
        for (int i = 0; i < meshPartsCount; i++)
        {
            ...
            for (int j = verticesInPart * i + (i > 0 ? -vertsInShape : 0);
                 (j < verticesInPart * (i + 1) && j < vertices.Length) ||
                 (i == meshPartsCount - 1 && j < vertices.Length);
                 j++)
            {
    ...
    public void AddNoiseToWidths()
    {
        for (int i = 0; i < controlPoints.Count; i++)
        {
            ...
            controlPoint.w = controlPoint.w +
                             (noiseWidth
                                 ? noiseMultiplierWidth * (Mathf.PerlinNoise(noiseSizeWidth * i, 0) - 0.5f)
                                 : 0);
    ...
    public void SimulateRiver(bool generate = true)
    {
            ...
            if (i > 0)
            {
                ...
                if (i % simulatedRiverPoints == 0 || simulatedRiverLength <= length || end)
                {
                    ...
                    if (generate)
                    {
                        ...
                        newPosition.w = widthNew + (noiseWidth
                            ? noiseMultiplierWidth * (Mathf.PerlinNoise(noiseSizeWidth * added, 0) - 0.5f)
                            : 0);
        ...
        if (!generate)
        {
            ...
            widthNoise = widthNew +
                         (noiseWidth ? noiseMultiplierWidth * (Mathf.PerlinNoise(noiseSizeWidth * i, 0) - 0.5f) : 0);
    ...
    public void ShowTerrainCarve(float differentSize = 0, bool useCurve = true)
    {
        ...
        for (int i = 0; i < carvePointsDown.Count - 1; i++)
        {
            ...
            rayPointDown = carvePointsDown[i];
            rayPointUp = carvePointsUp[i];
            ...
            if (terrainAdditionalWidth > 0)
            {
                for (int t = 0; t < detailTerrain; t++)
                {
                    ...
                    {
                        if (noiseCarve)
                            noise = Mathf.PerlinNoise(point.x * noiseSizeX, point.z * noiseSizeZ) *
                                noiseMultiplierOutside - noiseMultiplierOutside * 0.5f;
                        else
                            noise = 0;
            ...
            for (int t = 0; t <= detailTerrain; t++)
            {
                ...
                if (Physics.Raycast(point + Vector3.up * 500, Vector3.down, out hit, 10000, maskCarve.value))
                {
                    if (noiseCarve)
                        noise = Mathf.PerlinNoise(point.x * noiseSizeX, point.z * noiseSizeZ) *
                            noiseMultiplierInside - noiseMultiplierInside * 0.5f;
                    else
                        noise = 0;
    ...
    public void TerrainCarve(Terrain singleTerrain = null)
    {
        ...
        foreach (Terrain terrain in Terrain.activeTerrains)
        {
            ...
            for (pointsStart = 0;
                 pointsStart < pointsUp.Count;
                 pointsStart = Mathf.Clamp(pointsStart + pointsCount - 1, 0, pointsUp.Count))
            {
                ...
                for (int x = 0; x < heightmapData.GetLength(0); x++)
                {
                    for (int z = 0; z < heightmapData.GetLength(1); z++)
                    {
                        ...
                        if (meshCollider.Raycast(ray, out hit, 10000))
                        {
                            ...
                            if (debugLines)
                            {
                                Debug.DrawLine(hit.point, hit.point + Vector3.up * 0.1f, Color.magenta, 10);
                            }
                        }
                    }
                }
                    ...
                    for (int z = 0; z < alphamapData.GetLength(1); z++)
                    {
                        ...
                        if (meshCollider.Raycast(ray, out hit, 10000))
                        {
                            ...
                            if (!mixTwoSplatMaps)
                            {
                                ...
                                {
                                    if (minDist >= 0)
                                        noise = Mathf.PerlinNoise(hit.point.x * noiseSizeXPaint,
                                                    hit.point.z * noiseSizeZPaint) * noiseMultiplierInsidePaint -
                                                noiseMultiplierInsidePaint * 0.5f;
                                    else
                                        noise = Mathf.PerlinNoise(hit.point.x * noiseSizeXPaint,
                                                    hit.point.z * noiseSizeZPaint) * noiseMultiplierOutsidePaint -
                                                noiseMultiplierOutsidePaint * 0.5f;
                                }
                                else
                                    noise = 0;
                                ...
                                alphamapData[x, z, currentSplatMap] = Mathf.Clamp01(
                                    Mathf.Lerp(alphamapData[x, z, currentSplatMap], 1,
                                        terrainPaintCarve.Evaluate(minDist) +
                                        terrainPaintCarve.Evaluate(minDist) * noise));
                                ...
                                for (int l = 0; l < terrainData.terrainLayers.Length; l++)
                                {
                                    ...
                                    {
                                        alphamapData[x, z, l] = oldValue == 1
                                            ? 0
                                            : Mathf.Clamp01(alphamapData[x, z, l] *
                                                            ((1 - alphamapData[x, z, currentSplatMap]) /
                                                             (1 - oldValue)));
                                    }
                                }
                            }
                            else
                            {
                                ...
                                if (noise > 0)
                                {
                                    ...
                                    alphamapData[x, z, secondSplatMap] =
                                        Mathf.Clamp01(Mathf.Lerp(alphamapData[x, z, secondSplatMap], 1, noise));
                                    ...
                                    for (int l = 0; l < terrainData.terrainLayers.Length; l++)
                                    {
                                        if (l != secondSplatMap)
                                        {
                                            alphamapData[x, z, l] = oldValue == 1
                                                ? 0
                                                : Mathf.Clamp01(alphamapData[x, z, l] *
                                                                ((1 - alphamapData[x, z, secondSplatMap]) /
                                                                 (1 - oldValue)));
                                        }
                                    }
                                }
                                        ...
                                        for (int l = 0; l < terrainData.terrainLayers.Length; l++)
                                        {
                                            if (l != cliffSplatMap)
                                            {
                                                alphamapData[x, z, l] = oldValue == 1
                                                    ? 0
                                                    : Mathf.Clamp01(alphamapData[x, z, l] *
                                                                    ((1 - alphamapData[x, z, cliffSplatMap]) /
                                                                     (1 - oldValue)));
                                            }
                                        }
                                    }
                                        ...
                                        for (int l = 0; l < terrainData.terrainLayers.Length; l++)
                                        {
                                            if (l != cliffSplatMapOutside)
                                            {
                                                alphamapData[x, z, l] = oldValue == 1
                                                    ? 0
                                                    : Mathf.Clamp01(alphamapData[x, z, l] *
                                                                    ((1 - alphamapData[x, z, cliffSplatMapOutside]) /
                                                                     (1 - oldValue)));
                                            }
                                        }
                                    }
    ...
    float FlowCalculate(float u, float normalY, Vector3 vertice)
    {
        float noise =
            (noiseflowMap
                ? Mathf.PerlinNoise(vertice.x * noiseSizeXflowMap, vertice.z * noiseSizeZflowMap) *
                noiseMultiplierflowMap - noiseMultiplierflowMap * 0.5f
